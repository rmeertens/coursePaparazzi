...
Paparazzi only runs on Linux. If you have your own Virtualbox or Linux installed you can follow the instructions on this site: http://wiki.paparazziuav.org/wiki/Installation and watch this video: \url{https://www.youtube.com/watch?v=eW0PCSjrP78}.
...
We start by checking if the drone works by holding it in our hand very firmly while wearing safety glasses. Put the drone in ATT mode and arm the motors by putting the left stick to the lower-right position. If everything went well the motors should now spin slowly. Put the left stick up to give more throttle and verify that this works. Now try if pitch and roll work with the right stick. 
