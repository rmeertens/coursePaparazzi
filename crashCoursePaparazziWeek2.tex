...
Open a terminal (Ctrl+Alt+T) and start \fbox{\$ jstest-gtk}. Find your joystick in the list. If your device is not \verb"/dev/input/js0", write down the number after `js', you will need this later. Double-click on your joystick to start the test and calibration program.
...
\item{Open a terminal window and navigate to the paparazzi folder "\$ cd paparazzi". Add the mavlabCourse remote "\$ git remote add mavlabCourse https://github.com/tudelft/paparazzi", and update using "\$ git fetch mavlabCourse mavlabCourse2018"}
